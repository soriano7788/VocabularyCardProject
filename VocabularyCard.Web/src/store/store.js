    ...
    actions: {
        ...
        autoLogin: ({ commit, dispatch }) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                // 第一種(3 和 4)， access token 有效，直接 commit("setTokenData", tokenData); 後 resolve
                // 第二種(0 和 1)， 兩種 token 都過期了，或是連資料都沒有，只能重新輸入帳密登入了
                // 第三種(2)， access token 已過期， refresh token 還有效，所以就用 refresh token 去換新的 access token 回來就可以了
                ...
                if (tokenData == null) {
                    ...
                    console.log("isRefreshTokenInValidityPeriod: ", isRefreshTokenInValidityPeriod);
                    console.log("isAccessTokenInValidityPeriod: ", isAccessTokenInValidityPeriod);
                ...
                // 呼叫一次 resolve 後，並無法終止下面程式碼繼續執行，
                // 必須確保各 resolve 只會在各自的情況下被呼叫一次，
                // 不可在一個 case 中，多個 resolve 被呼叫
